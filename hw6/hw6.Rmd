...
# This summary displays the mean (average), median (middle value in the dataset), 1st Quartile (the median for the first 25% of the data), 3rd Quartile (the median for the last 25% of the data - the upper 75%), and the minimum and maximum values for each Water, Protein, Fat, Lactose, and Ash content in the Mammals_Milk data.  
```
...
```{r}
Mammal_Euclidean <- Mammals_Milk_Std
Mammal_Euclidean <- dist(Mammal_Euclidean)
plot(hclust(Mammal_Euclidean, method = 'single'), lwd=3, main = 'Mammals Clustered by Std. Milk Composition \n Euclidean Metric, Complete Linkage', sub="", xlab = 'Mammals', cex.lab=1.25, hang=-1, frame.plot=TRUE)

# the Euclidean metric is a way to find the distance between two points.  Specifically, it would solve by ((y2-y1)^2 + (x2-x1)^2)^1/2.  This takes the square root of the added values to the difference between x and y's squared.  This dendrogram shows these distances using a hierarchical cluster.
```
...
Mammals_Milk <- data.frame(Mammals_Milk) %>% mutate(km_3 = MMilk_km_3$cluster) %>% mutate(km_4 = MMilk_km_4$cluster) 
Mammals_Milk %>% arrange(km_3) 
Mammals_Milk %>% arrange(km_4)

# k clustering is a popular way to form clusters in which k is the number of clusters you wish for.  This data frame adds a new column showing the values for k = 3 and k = 4.
```
...
```{r}
MMilk_Agg_K3 <- Mammals_Milk %>% group_by(km_3) %>% summarize(Count = nrow(Mammals_Milk), Mean_Water = mean(Water), Mean_Protein =mean(Protein), Mean_Fat = mean(Fat), Mean_Lactose = mean(Lactose), Mean_Ash = mean(Ash)) %>% round(2)
MMilk_Agg_K4 <- Mammals_Milk %>% group_by(km_4) %>% summarize(Count = nrow(Mammals_Milk), Mean_Water = mean(Water), Mean_Protein =mean(Protein), Mean_Fat = mean(Fat), Mean_Lactose = mean(Lactose), Mean_Ash = mean(Ash)) %>% round(2)
MMilk_Agg_K3
MMilk_Agg_K4

# this code summarizes and adds columns which display the mean/average milk, water, protein, fat, lactose, and ash in Mammals milk.
```
# Question 5
...
#these box plots display the means, medians, upper quartile, lower quartile, and outliers.  The different graphs show the different y values, depending on if it measuring and showing water, protein, fat, lactosse, or ash. 
```
...
# I suggest that 4 is a good number of clusters.  I came to this conclusion through the elbow method.  One can see that the graph curves or creates an elbow at 4.  This means that the y values following, or greater than 4, change the slope drastically due to it rounding off and being similar in value.  Therefore, 4 is the best number of clusters. 
# As compared to the dendrogram in question 2, one sees that the y axis values do not even go up to 4.  This can be for a multitude of reasons, however, this causes me to think that the dendogram does not fully display some necessary data.  I would follow the number of clusters being 4, instead of a value lower than 2 - which the dendrogram suggests.
