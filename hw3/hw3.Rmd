...
```{r}
scatterplot <- ggplot(Cars93, aes(x = EngineSize, y = MPG.city)) +
  ...
  labs(
    x = 'Engine Size', 
    y = 'City MPG', 
    title = 'Engine Size for MPG.city', 
    subtitle = 'Scatterplot of the Engine size and the Miles Per Gallon in City'
  )
...
```{r}
scatterplot1 <- ggplot(Cars93, aes(color=Type, x = EngineSize, y = MPG.city)) +
  geom_point(size = 2) + 
  xlim(0.5,6) +
  ylim(5, 50) +
  labs(
...
```{r}
scatterplot2 <- ggplot(Cars93, aes(color=Type, x = EngineSize, y = MPG.city)) +
  geom_point(size = 2) + 
  xlim(0.5,6) +
  ylim(5, 50) +
...
```{r}
barplot_1 <- ggplot(Cars93, aes(AirBags, fill = AirBags)) +
      geom_bar() +
      labs(title = 'Number of Airbags per Car', 
           x = 'Number of Airbags', 
           y = 'Frequency', 
           caption = 'Number of Airbags per Car in Cars93 Data')
...
```{r}
barplot_2 <- ggplot(Cars93, aes(x='Origin', fill = AirBags)) +
      geom_bar(position = 'dodge') +
      labs(title = 'Number of Airbags per Car by Origin', 
           x = 'Origin', 
           y = 'Frequency', 
           caption = 'Number of Airbags per Car in Cars93 Data')
...
```{r}
Australia = ggplot(au_states) +
  geom_sf() +
  labs(
    title = "Map of Australia",
    caption = "Australia using longitude and latitude"
  )
...
```{r}
Australia2 = ggplot(au_states, aes(fill=NAME)) +
  geom_sf() +
  labs(
    title = "Map of Australia",
    caption = "Australia using longitude and latitude"
...
```{r}
Australia3 = ggplot(au_states) +
  geom_sf(aes(fill=NAME)) +
  geom_point(data = au_capitals, col= 'red', size = 2, aes(x=lon, y=lat)) +
  labs(
    title = "Map of Australia",
    caption = "Australia using longitude and latitude"
  )
...
```{r}
mapview(au_states)+
  mapview(capitals_geo, col.regions = 'red', legend = F)
```
