...
output: 
  html_document
---
16/20. Any work done by hand should be shown here. 
...
```{r}
Vans_Euclidean <- dist(Vans93[,2:6])
Vans_Euclidean
...
# max_distance = (Lumina_APV, Eurovan)
# min_distance = (Caravan, Aerostar)
```
...
```{r}
Vans_Manhattan <- dist(Vans93[,2:6], method = "manhattan")
Vans_Manhattan
Vans_Manhattan1 <- as.matrix(Vans_Manhattan)
colnames(Vans_Manhattan1)<-Vans93[,1]
rownames(Vans_Manhattan1)<-Vans93[,1]
round(Vans_Manhattan1, 2)
Vans_Manhattan1
...
b) Farthest-neighbor
```{r}
# 9 - work done on paper
# Show your work here.
```

c) Average-neighbor
```{r}
# 5.02 - work done on paper
# Show your work here. (-2)
...
```{r}
#vertebrate <- read.csv('/Users/jennanowland/Desktop/Jenna Data 180/DATA-180-Jenna-Nowland/hw5/vertebrate.csv')
...
plot(hclust(Ver_Euclidean, method = "complete"), lwd =3.5, main = "Euclidean Clustered vertebrate", xlab='Vertebrates', cex.lab=1.25, sub = '', hang = 1)
...
plot(hclust(Ver_Manhattan, method = "single"), lwd =3.5, main = "Manhattan Clustered vertebrate", xlab='Vertebrates', cex.lab=1.25, sub = '', hang = 1)
```

Which method do you prefer?
```{r}
# I prefer the single linkeage due to it avoiding that "busy" look.  The single linkeage is noticabley easier to read.
```
